...
title: C++ 教學系列 ── Class
date: 2022-04-28
abstract: Class 背後最核心的概念是資料的抽象化與資料封裝。這些是一種程式設計上的技巧、想法，其清楚區分了介面和實作的不同，介面是與使用者直接互動的運算，實作則是利用了 Class 內的資料成員與成員函式來寫出的演算法，而封裝則是強制分開了介面與實作，實作會被隱藏，使用者只看的到介面，但看不到實作
