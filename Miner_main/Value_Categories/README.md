...
值類別是 C\+\+11 新產生出來的新文法，其實早在 C\+\+03，甚至是 C\+\+98 時就已經有類似的定義了，不過這些定義在 C\+\+11時進行了大改動，並且在 C\+\+17 時，由於 Copy Elision 與 TMC 的緣故，定義又有一次小改動，本文會以 C++17 的定義為主。 而由於這是給編譯器看的，是語意的部分，所以有時用組語會看不出個所以然，也因此這個東西十分的抽象
...
移動語意是一個很大很廣的概念，在看移動語意前我們需要知道「值類別（Value Category）」是什麼。 而若要知道什麼是值類別，我們需要先分清楚「變數（Variable）」、「運算式（Expression）」、「陳述式（Statement）」的差別。 這些東西比較偏向程式的文法，之前在 [Object、Expression、Statement](https://github.com/Mes0903/Cpp-Miner/tree/main/Miner_Tutorial/Object_Expression_Statement) 中已經有詳細講過了，如果你沒看過也沒關係，下面會幫大家簡單介紹一下
...
::: tip  
變數是一種物件（object），或者是 non-static data member 的 reference（[來源](https://timsong-cpp.github.io/cppwp/n4950/basic.pre#6)）  
:::

一般來說，「變數」是個內容可更改的物件（除非我們加了 cv 限定詞等綴詞），可以拿來接運算式回傳的值。 另外變數並不一定要有記憶體位址，因為 C++11 後變數的定義多了 Reference，而 Reference 可能沒有記憶體位址。 簡單來說，變數只是一個在程式中，能夠存取一個值的識別字（identifier），至於實際上有沒有 memory allocate 則不是這麼重要，因為這很看編譯器優化與作業系統，但他肯定會有 logic address

::: tip  
這邊的物件照著 C++ 標準的規定，在 C++ 中，物件（object）一定要有：
...
在沒有編譯器優化的情況下，由於變數（物件）的宣告會同時擁有定義，因此宣告一個變數（物件）的時候編譯器會在記憶體裡面挖一塊空間，並在這塊記憶體空間上加上識別字（identifier），如此一來，我們便能通過這個 identifier 對這個記憶體的位址操作，像是賦值之類的，看看這個例子：
...
一個運算式由一個以上的運算元（operand）和運算子組成，用來表示一個計算式，被計算後它可能會「回傳一個結果」，可能還會有額外的作用（side-effects），像是 `std::printf("%d", 4)` 的副作用會呼叫 system call，印出一個 4

讓我們看看下面這些例子：

```cpp
42    // 一個常數的運算式，回傳 42
17 + 42    // 一個加法的運算式，回傳 59
...
sizeof a    // 一個計算 byte size 的運算式，回傳「運算式」a 的 byte size
[]{return 3;}    // 一個回傳 3 的 lambda 運算式
printf("Hi!")    // 一個有額外作用的運算式，回傳值被捨棄了(discarded-value)，其實它的回傳值是輸出的字數
```

注意我都沒寫分號，因為運算式是一個「組件」，一但加上分號，他就不是一個「組件」，而是一個陳述句了。 也因此上面這個程式肯定是無法正常運作的，因為他們都不是陳述句，程式運作的是陳述句

運算式主要有分「主運算式（Primary Expression）」、「非計算運算式（Unevaluated expressions）」與「棄值運算式（Discarded-value Expression）」，但這不是今天我們的主軸，忘記的話可以回去看一下之前的 [Object、Expression、Statement](https://github.com/Mes0903/Cpp-Miner/tree/main/Miner_Tutorial/Object_Expression_Statement)

### 陳述句 Statement

一個陳述句是程式執行的基本元素，通常最後面會有 `;` 來結尾。 陳述句會照順序被執行，它可能會單獨一行出現，但我們也可以利用 `{}` 將許多陳述句包成一個新的陳述句，這個我們稱它為「複合陳述句（Compound Statement）」。 而運算式通常會是陳述句的一部分，另外還有一種陳述句叫「運算陳述句（Expression Statement）」，這種陳述句裡面只有運算式
...
然後我們將運算陳述句拿出來講一下，因為要分清楚運算式跟陳述句的差別，運算陳述句的結構長這樣：

> attr<sub>(optional)</sub> expression<sub>(optional)</sub> ;

`attr` 是[屬性識別字](https://en.cppreference.com/w/cpp/language/attributes)，用來告訴編譯器一些事情，你可以想像成給編譯器看的註解，可加可不加

而 `expression` 就是前面提到的運算式，「可加可不加」，如果不加就會變一個空陳述句，也就是只有一個分號的陳式句，空陳述句通常用來提供 for、while 迴圈一個空的代碼塊（body），或是一個標籤（label）的代碼塊

程式裡大部分的陳述句都是運算陳述句，像是賦值或是函式的呼叫等都是，其實就只差了一個分號，因此一個可執行，一個不行。 其他的大家有興趣可以到 [cppreference](https://en.cppreference.com/w/cpp/language/statements)上看

## 值類別 Value Categories

現在我們知道變數、運算式和陳述式差在哪了，接下來就可以討論什麼是 Value Categories 了，首先是最重要的一點：值類別是用來判斷<strong><span class = "yellow">運算式的種類</span></strong>的，而不是用來判斷物件的。 這整篇文章你可以都看不懂，但是一定要知道這件事，他用來判斷<strong><span class = "yellow">運算式的種類</span></strong>
...
1. 這個運算式的結果有恆等性（identity）嗎？
2. 如果該結果有恆等性，那它的值可以安全的「移動」給別人嗎？

#### 1. 這個運算式的結果有恆等性嗎？

首先我們先來看一下，什麼叫做「有恆等性」呢？ 有恆等性表示我們有辦法能證明「這個 expreesion 的結果與另一個 expression 的結果是同一個」，這不是指值相等，而是指它們是否指向同一個東西。 最常見的驗證方法就是取址，我們看一下這個例子幫大家理解一下什麼叫做「有恆等性」：
...
這是個非常直接的例子，因為 `a` 是個有記憶體位址的變數，所以上下兩行 `a` 這個 expression 回傳的物件都是「變數 `a`」，我們透過取址來證明了這件事

「恆等性」是口語上的概念，用來描述「這個 expression 到底指向誰」，標準僅在文字敘述中使用該詞，並沒對它做定義
...
注意上例中都沒加 `;`，因為是在討論 expression 的結果。 很多人會分不清楚 expression 與物件，因為它們共用了同一個名稱，以第一個 expression 來說：

```c
a
```

這個 `a` 是一個 expression，你可以想像成（事實上編譯器也是這麼做的），他會經過運算（evaluation），以這邊來說是空運算（沒做任何事），接著查找名稱為 `a` 的物件，並將它回傳。 換句話說你可以把 expression 理解成一個「動作」，這個動作最後會回傳一個物件

再看一個 `a + 2` 這個例子，首先它看到 `+` 號所以知道要這個 expression 的運算要做加法，接著它會去查找名稱為 `a` 的物件，取得它的值，並將該值加 2，然後回傳。 因此回傳的是一個值，而非物件，自然沒有恆等性

如果想看更實際的編譯器操作過程，可以找找像是 LVN 這類演算法的實作來看

#### 2. 如果該結果有恆等性，那它的值可以安全的「移動」給別人嗎？

再來看第二點，那麼什麼時候該值可以安全的「移動」給別人呢？ 如果這個物件是個匿名的「暫時」物件，或者這個的生命週期快要結束了，我們就稱它可以安全的「移動」給別人，讓我們看個例子：
...
`func()` 會回傳一個字串，而我們想要將這個回傳的字串推入 `vector`，現在問題來了，這個字串能不能安全的「移動」給別人呢？ 如果可以，那麼我們將可以少一次的複製，因為我們可以直接將這個字串「移動」進 `vector`，而不是複製一個一樣的字串後推入 `vector`。 而那麼這邊的答案是可以！ 因為 `func()` 回傳的字串在 `push_back()` 後就要被解構了，因此我們可以將它的資源接收過來，而不是複製後再將 `func()` 的字串解構

上例的接下來我們有個 `std::string x`，並且想將它「移動」進 `vector`，原本不應該可以移動的，因為 `x` 等等並不會被解構，但我們可以利用 `std::move(x)` 來讓他強制「移動」

原則上來講，如果一個物件僅僅是為了「傳遞」資訊，或「暫時存取」某個東西，而不是用來「儲存」某些資訊的，那麼他就不可被「移動」

到此為止，你知道了什麼是恆等性，也知道了什麼時候可以安全地對物件的值做移動，接下來我們就可以開始看什麼是 Lvalue、Prvalue 與 Xvalue 了！

## Lvalue Expression

Lvalue，中文翻作左值，一個 Lvalue Expression 回傳的結果具有恆等性，並且在正常的情況下它不該被「移動」，只能複製。 這代表一個 Lvalue Expression 回傳的結果會有一個記憶體位址，基本上就是指物件。 我們能夠透過這個名稱，在生成它的 statement 之後來使用這塊記憶體內的東西
...
注意上面我都沒有標上 `;`，因為這邊是在講 expression 的分類，而不是 statement，也不是在講物件本身

### 種類

Lvalue Expression 有以下這些（[來源](https://en.cppreference.com/w/cpp/language/value_category#lvalue)），我後來發現 cppreference 上面多很多例子，可以點進去看一下：

- 變數的名稱、函式本體、[模板參數物件（C++20）](https://en.cppreference.com/w/cpp/language/template_parameters#Non-type_template_parameter)、或一個資料成員，像是 `std::cin` 或 `std::endl`。 就算變數本身的型態為 rvalue reference，包含其名稱的 expression 仍為一個 lvalue expression
...
- 回傳 Lvalue Reference 的運算子重載或函式呼叫（function call），如 `std::getline(std::cin, str)`、`std::cout << 1`、`str1 = str2` 或 `++it` 等
...
- 利用了[賦值運算子或複合賦值運算子](https://en.cppreference.com/w/cpp/language/operator_assignment)的運算式，如 `a = b`、`a += b`、`a %= b`

- 利用了[前置遞增、遞減運算子](https://en.cppreference.com/w/cpp/language/operator_incdec.html#Built-in_prefix_operators)的運算式，如 `++a` 與 `--a`

- 利用了[間接取值運算子](https://en.cppreference.com/w/cpp/language/operator_member_access#Built-in_indirection_operator)的運算式，像是 `*p`

- 利用了[注標運算子](https://en.cppreference.com/w/cpp/language/operator_member_access#Built-in_subscript_operator)的運算式，像是 `a[n]`、`p[n]`，但注意 `a` 要是一個陣列左值，不能是[陣列右值](https://en.cppreference.com/w/cpp/language/array#Array_rvalues)
...
- 利用了[成員物件訪問運算子](https://en.cppreference.com/w/cpp/language/operator_member_access#Built-in_member_access_operators)的運算式，像是 `a.m`，但 `m` 不能是 `enum` 的成員、某些非靜態的成員函式，或一個 class prvalue 內的非靜態成員（以上例來說就是 `a` 是 class prvalue 且 `m` 是非靜態成員）
	...
	// x 可以是 xvalue 或 prvalue 的參考 (因為他是 rvalue reference)
	void f( int( &&rr )[2][3] ) {
	    std::cout << " " << sizeof( rr ) << '\n';
	}
    ...
    ```
    看不懂的話可以等後面的 TMC、Copy Elision、RVO 讀完後再回來看一次
...
- 部分利用了[三元運算子](https://en.cppreference.com/w/cpp/language/operator_other#Conditional_operator) 的運算式

- 字串常量（string literal），像是 `"Hello World"`  
   - 因為字串常量其實像是一個靜態成員，在整個程式結束前都會被存在某個唯讀的記憶體區段

- 轉型成 lvalue reference 型態的運算式，如 `static_cast<int&>(x)` 與 `static_cast<void(&)(int)>(x)`

- lvalue reference 型態的 constant template parameter
...
- 回傳「函式」的 rvalue reference 的函式呼叫，如

- 轉型成「函式型態」的 rvalue reference 的運算式，像是 `static_cast<void (&&)(int)>(x)`

### 特性

那麼 Lvalue Expression 有一些特性，有些是只有 Lvalue Expression 有的，有些是 Xvalue Expression 也有的（換句話說就是 glvalue 有的）

#### 僅限 Lvalue Expression 擁有的特性

- 可以被取址

    取址運算符 `&` 的後方必須要接上 Lvalue Expression 或一個 [Qualified-id](https://en.cppreference.com/w/cpp/language/identifiers#Qualified_identifiers) （[來源](https://timsong-cpp.github.io/cppwp/n4659/expr.unary.op#3)），如 `&std::hex` 為合法的操作

- 一個可更改的 Lvalue Expression 可以擺在賦值運算子和複合賦值運算子的左邊

- Lvalue Expression 可以拿來初始化 lvalue reference，這個動作會給被連結到的物件一個新的識別字（identifier）

#### Xvalue Expression 也有的特性

- 可被隱式轉換成 prvalue，像是「Lvalue 與 Prvalue 的隱式轉換」、「陣列與指標的隱式轉換」、「函式與指標的隱式轉換」
...
- 可能是[多形](https://en.cppreference.com/w/cpp/language/object#Polymorphic_objects)的，它標識的對象的[動態類型](https://en.cppreference.com/w/cpp/language/type#Dynamic_type) 不一定是表達式的靜態類型
    ...
    如果某個 Lvalue Expression 或 Xvalue Expression 指向了[多形](https://en.cppreference.com/w/cpp/language/object#Polymorphic_objects)的物件，那麼它衍生的型態我們稱它為[動態類型](https://en.cppreference.com/w/cpp/language/type#Dynamic_type)：
...
Xvalue，中文翻作「將亡值」，一個 Xvalue Expression 回傳的物件具有恆等性，但是它通常用來「暫時儲存」某個東西。 這代表一個 Xvalue Expression 回傳的物件會有一個記憶體位址，但我們無法在「生成它的 statement」之外來使用這塊記憶體內的東西
...
在 `main` 內有兩行同樣的 Expression `X().i`，這個 Expression 會生成一個暫時的匿名物件，所以我們可以看到輸出裡面第一個 `X().i` 執行了建構之後馬上又解構了，如果我們沒有用 identifier 去抓住它，它就消失了，因為它是一個暫時物件，所以這個 Expression 為 Xvalue Expression。 這邊我呼叫了兩次，大家可以看見上下兩行的暫時物件的位址不一樣，他們不是同一個物件，我們無法在「外部呼叫」它

### 種類

那 Xvalue Expression 有以下這些（[來源](https://en.cppreference.com/w/cpp/language/value_category#xvalue)）：

- 部分利用了[成員物件訪問運算子](https://en.cppreference.com/w/cpp/language/operator_member_access#Built-in_member_access_operators)的運算式，像是 `a.m`，但 `a` 要是 rvalue 而且 `m` 是一個非靜態的資料成員

- 部分利用了指向資料成員的[成員指標運算子](https://en.cppreference.com/w/cpp/language/operator_member_access#Built-in_pointer-to-member_access_operators)的運算式，像是 `a.*mp`，其中 `a` 是一個 rvalue 且 `mp` 是一個資料成員的指標

- 利用了[逗號運算子](https://en.cppreference.com/w/cpp/language/operator_other#Built-in_comma_operator) 的運算式，像是 `a, b`，其中 `b` 為 xvalue

- 部分利用了[三元運算子](https://en.cppreference.com/w/cpp/language/operator_other#Conditional_operator)的運算式

- 回傳「物件」的 rvalue reference 的函式呼叫或運算子重載表達式，像是 `std::move(x)`
...
- 轉型成「物件」的 rvalue reference 的運算式，像是 `static_cast<char&&>(x)`

- 經過 [temporary materialization](https://en.cppreference.com/w/cpp/language/implicit_cast.html#Temporary_materialization)（TMC）後產生出暫時物件的運算式
  - TMC 是一個挺重要的東西，後面會提到

- [move-eligible expression](https://en.cppreference.com/w/cpp/language/value_category.html#Move-eligible_expressions)（C\+\+23）
...
#### 與 Lvalue Expression 共用的特性

- 可被隱式轉換成 prvalue，像是「Lvalue 與 Prvalue 的隱式轉換」、「陣列與指標的隱式轉換」、「函式與指標的隱式轉換」

- 可能是[多形（Polymorphic）](https://en.cppreference.com/w/cpp/language/object#Polymorphic_objects)的，它標識的對象的[動態類型](https://en.cppreference.com/w/cpp/language/type#Dynamic_type)不一定是表達式的靜態類型
...
Prvalue，中文翻作純右值，一個 Prvalue Expression 回傳的是一個常量，或是沒有恆等性的物件，在定義上 Prvalue 可以被移動
...
- Xvalue Expression 和 Prvalue Expression 可以拿來初始化「唯讀」的 lvalue reference，如此一來該暫時物件的生命週期會被延長，直到這個 reference 所處的範圍（scope）結束

- Xvalue Expression 和 Prvalue Expression 可以拿來初始化 rvalue reference，如此一來該暫時物件的生命週期會被延長，直到這個 reference 所處的範圍（scope）結束
...
參考並不是我們今天的主軸，但由於 Rvalue Reference 對理解 Value Categories 挺重要的，所以我這邊稍微提一下 Lvalue Reference 和 Rvalue Reference，至於 Forward Reference 就太遠了，所以這邊就不提了，詳細的可見 [Reference](https://github.com/Mes0903/Cpp-Miner/tree/main/Miner_Tutorial/Function_Memory#%E4%BB%80%E9%BA%BC%E6%98%AF%E5%8F%83%E8%80%83-reference-) 一節
...
參考並不一定會有 memory allocate，而且參考也不是一個 object，這樣才符合他的精神：「別名」。 儘管為了實現語意，編譯器通常會給他 memory allocate，看看這個[例子](https://stackoverflow.com/questions/45821678/is-always-the-address-of-a-reference-equal-to-the-address-of-origin)：
...
我們透過「僅有單一成員的 struct 的位址、大小與第一個元素相同」的特性將 `ref` 的位址與 `size` 給印了出來，算是有點破壞規則，但在這個情況下，將編譯器的優化關掉，他就會擁有 memory location，如此一來我們便能看見 Reference 的位址了，記得在寫程式的時候別這樣做呀XD 不然就違反 Refernce 的精神了，這只是個方便大家思考的例子

再來，參考一定要被初始化，來看下面這個[例子](https://timsong-cpp.github.io/cppwp/n4950/dcl.init.ref)：
...
一但 reference 被初始化，我們就無法再對 reference 本身進行操作了。 我們也不能有參考的參考（reference to reference），參考的陣列（arrays of reference），和參考的指標（pointer to references），因為 reference 並不是物件（[來源](https://timsong-cpp.github.io/cppwp/n4950/dcl.ref#5)）

這樣看起來，一旦參考被初始化後，參考「本身」好像就消失在程式當中，真的變成一個「別名」了，那麼底層是如何實作的呢？ 這不一定，通常是利用指標來實作，但要記得，他不一定要是指標，參考與指標本質上是不同的東西，僅僅是因為他們倆個的性質很相似而已

那麼，編譯器知道參考本身的存在嗎？ 答案是知道：
...
可以看見 Compiler 可以分出來兩者的差別，因為兩者的 type 是不同的。 如果對 Referecne 的定義與相關的詳細規範還有興趣，可以到[這裡](https://timsong-cpp.github.io/cppwp/n4950/dcl.ref)看看
...
> & attr<sub>(optional)</sub> declarator

例如 `int &r = a;`，當中的 `r` 就是一個 Lvalue Referecne，型態是 `int&`。 那麼他有幾個用處：
...
因為 Lvalue Reference 只能綁定到一個 Lvalue Expression 回傳的物件，從語意上來講，更改暫時物件的值不一定是合理的，而常量值更不用說了，他甚至可能不是個物件，沒有儲存位址，要怎麼改動他呢？ 由於 Lvalue Refference 綁定到的物件基本上可以被更改（除非他有加上 const 限定詞），所以我們無法用它綁定 Prvalue Expression 與 Xvalue Expression（統稱 Rvalue）

那麼這會出現什麼問題呢? 最明顯的問題出在函式使用 call-by-reference 時：
...
怎麼解決呢？ 照剛剛的邏輯，不能利用 Lvalue Reference 做連結的原因是因為 Prvalue 與 Xvalue 回傳的物件照理說不應該被更改。 那麼只要我們保證不更改他就好了對吧！ 所以答案是「加上 const」：
...
`++a` 被擋了下來，這也很正常，因為它是唯讀的，那麼要怎麼做才可以讓它像這樣做連結呢：
...
但這樣就回到了一開始的問題，常量並沒有記憶體位址，語意上來說不該能被更改，我們會說它能更改是從臨時物化與記憶體的角度去看，但在寫程式的時候我們應該由語意方面來寫，而不是底層，因為還有編譯器優化與平台差異的問題，底層應該交給他們去判斷。 於是陷入了死胡同，直到 C++11 時為了滿足移動語意，Rvalue Reference 的出現以及值類別、TMC、Copy Elision 等更詳細的定義，這個問題才被解決
...
Copy Elision 在 C\+\+11 與 C\+\+14，甚至更早期的 C\+\+03、C\+\+98 時就存在了，不同的是 C++11 有移動建構子的出現，導致能夠省略的建構子又多了一種。 之後 Copy Elision 在 2015 年時，於 [P0135R0 提案](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2015/p0135r0.html)中被提出，說是應該要在某些情況下保證 Copy Elision 的發生。 後來提案被採用，在 C\+\+17 時「部分」保證 Copy Elision 會發生，就算建構子與解構子有 side effect 也一樣
...
基本上 Copy Elision 的精神就是在條件允許下，直到必須建構前，都不能建構，在初始化物件時如果能直接以傳入的原料（Value）來初始化就直接初始化，不要在那邊搞一堆複製或移動。 另外在下方，當我講 Copy Elision 的時候指的就是 Copy / Move Elision 了，因為習慣所以仍只講 Copy Elision，若是特殊情形我會特別標註，但應該是沒有啦
...
#### 環境 1：C\+\+14，關掉 Copy Elsion（`-std=c++14 -fno-elide-constructors`）
...
#### 環境 2：C\+\+17，關掉 Copy Elsion（-std=c++17 -fno-elide-constructors）
...
#### 環境 3：C\+\+17，打開 Copy Elsion（`-std=c++17`）
...
那我們現在就來看看這神奇的東西什麼時候會發生吧！首先是「保證」會發生的情況（[來源](https://timsong-cpp.github.io/cppwp/n4950/class.copy.elision)），在這些情況下，編譯器必須省略複製與移動建構子的呼叫，即使它們有 side effect 也一樣：

- 對於一個回傳 `T` 型態的 [return statement](https://en.cppreference.com/w/cpp/language/return)，像是函式，如果回傳的是一個相同型態 `T` 的 Prvalue Expression（無視 cv 限定詞），那麼他必須發動
    ...
    要注意的是他不一定需要移動建構子與複製建構子，因為我們沒用到他（看起來很像廢話），你可以把他 `delete` 掉試試看，但要有預設的建構子和解構子，不然他沒辦法建構解構（因為會用到）
...
-   再來是一個大重點，在初始化物件時，如果初始化器（initializer）內是一個相同型態的 Prvalue Expression（無視 cv 限定詞），那麼 Copy Elision 也保證發生，而且他必須直接初始化那個物件，不能複製和移動
    ...
    `T x = f();` 中的 `x` 會直接被初始化，和 `T x;` 等價，而 `T y;` 和 `T y = T( T( f() ) );` 等價

    而 Prvalue Expression 的定義也因為這項規定而改變了，還記得「回傳非參考類型的運算子重載運算式或函式呼叫（function call）」被歸類在 Prvalue Expression 嗎？ 以上方的例子來說就是 `f()`

    看到這裡你應該覺得很合理了，因為「沒有暫時物件」了，就好像常量一樣，也因此，我們會說 C\+\+17 後 Prvalue Expression 不可被移動，因為根本沒有暫時物件生出來給我們移動，並且依然沒有恆等性（因為沒有物件產生）

    要注意的是被初始化的物件不能是[潛在重疊的物件](https://timsong-cpp.github.io/cppwp/n4950/intro.object#7)，潛在重疊的物件包含：
...
那這邊我就大概列一列，翻譯一下 Cppreference 就好（[來源](https://en.cppreference.com/w/cpp/language/copy_elision#Non-mandatory_elision_of_copy.2Fmove_.28since_C.2B.2B11.29_operations)），因為我覺得最重要的是 NRVO，但 NRVO 後面會單獨拉出來講，所以就先不自己寫例子或找例子了，spec 裡面有給一些例子，有興趣的話可以點進[這裡](https://timsong-cpp.github.io/cppwp/n4950/class.copy.elision)看看

另外這個很看編譯器，然後我不確定官方是不是把所有有可能的狀況都列下來了，但我想不太可能。 主要項目如下：

- 命名回傳值優化（NRVO），這等後面談回傳值優化的時候再一起講

- 某些條件下的 `throw` 表達式，這是一種語言允許的拋出優化（throw optimization）

    在 [throw 表達式](https://en.cppreference.com/w/cpp/language/throw)中，若運算元是具有自動儲存期（automatic storage duration）的 non-volatile 物件 `obj` 的名稱（該物件不是函式參數或處理器參數），且 `obj` 所屬的作用域不包含最內層包住該 throw 的 [try block](https://en.cppreference.com/w/cpp/language/try.html)（如果有的話），則可以省略將 `obj` 複製初始化為例外物件（exception object）的步驟，而是直接用 obj 建構例外物件

- 在一個 handler（例如 catch 區塊）中，若程式的語意不會改變（除了處理器參數的建構與解構函式是否會執行以外），則可以省略對處理器參數的複製初始化，將該參數視為例外物件的別名

- 在 [coroutine](https://en.cppreference.com/w/cpp/language/coroutines.html) 中，可以省略 coroutine 參數的一份複製。 如果這樣做不會改變程式的語意（除了該參數複製物件的建構與解構函式是否會執行以外），則對該複製物件的引用會被替換成對對應 coroutine 參數本身的引用

### Copy / Move Elision 小整理

恭喜你看到這裡了，但後面還很長，這裡先幫大家整理一下，並且分享一個好用的小東西
...
這邊有一個要注意的地方是我們也有實作 const Lvalue Reference 的版本，但他傳入的是 Rvalue Reference 的版本，這代表在 `return` 的時候有「暫時物件」的產生！ 這邊我給了一個簡單的 code 讓大家看，[測試連結](https://godbolt.org/z/To7198TGa)：
...
我省略了很多繁瑣的過程，但大致上就長這樣。 如果你有自己在建構子和解構子裡面加上輸出，會看到輸出長這樣，然後我幫大家加一下註解：
...
如果有比較敏銳的朋友可能會發現這是我們說 Prvalue Expression 沒有恆等性的原因，因為事實上有兩個不同的物件，就算在建構子裡面輸出了 `this`（B 的位址），與 Rvalue Reference 輸出的位址也不相同（一個是 `&X`，一個是 `&B`），當然在我們不應該用這個方式來思考文法的問題，只是幫助大家理解而已。 那麼 C\+\+17 後的 Prvalue 又是另一件事，這剛剛解釋過了

另外可能有人會發現如果有實作 Rvalue Reference 的版本，那麼他會優先呼叫這個版本的建構子，原因牽涉到隱式移動（implicit move），後面再講，這邊只是為了讓大家知道建構解構的詳細過程而已
...
超方便的，上方名稱空間裡面的內容是用來判斷值類別的主體，而下方的巨集函式則幫助我們輸出
...
會到這邊才講是因為知道 Copy Elision 後會比較好理解它在幹嘛，雖然原因可能有點不一樣就是了，這邊利用了 `decltype` 來幫忙，但不是今天的主題，就先不展開了
...
基本上 TMC 的精神就是在必要的時候，一個完全型（Complete Type）的 Prvalue 能夠被轉換成同型態的 Xvalue。 TMC 會透過計算評估相關的 Prvalue Expression 來初始化一個相同型態的暫時物件當作它的結果，也就是說 TMC 是一個從 Prvalue 到 Xvalue 的橋樑。 前面的 Copy Elision 負責減少實例，而 TMC 則幫忙於必須要有實例的時候建構實例，看看這個[例子](https://godbolt.org/z/hbexrMKTc)，如果你想複習 Copy Elision，也可以把版本改回 C\+\+14 去看看：
...
- 拜訪或使用了 Class Prvalue 的成員時：
    ...
    `C()` 是個 Prvalue，這大家應該不陌生，前面的例子也解釋過了，當我們拜訪它的成員時，TMC 會產生一個暫時物件，這也是我們說 `C().i` 是 Xvalue 的原因
...
那麼 RVO 分成兩種，URVO 和 NRVO，兩者的差別是函式內回傳的物件有沒有 identifier，換句話說就是「是不是一個匿名物件」。 那麼我們就來看看吧！
...
在一個回傳語句中，當我們回傳一個與語句回傳型態同型態（無視 cv 限定詞），自動儲存週期的有名物件，簡單來說，函式回傳 `int` 而我們也真的 `return` 一個 `int`，而且這個回傳的物件不是被吃進來的參數或 catch clause 的參數，那就可能會發動 NRVO 來省略複製

NRVO 有沒有發動實際上還是看編譯器與 ABI，標準只講了哪些時刻不能發動，所以前面講 Copy Elision 時我們才會說 NRVO 不保證會發
...
- 回傳的物件為函式吃進來的參數時（[連結](https://godbolt.org/z/fMEKa48GW)）：
...
講了這麼多，那如果 Copy Elision 和 NRVO 都沒有發動的話怎麼辦呢？ 不用擔心，標準告訴我們，在 C\+\+11 之後，於回傳語句中，如果在某些特殊情況下，Copy Elision 應該要發動但沒發動，或是可能可以發動但還沒被保證，所以沒發動，像是我們在 C\+\+14 的版本內把回傳優化關掉的狀況。 這種時候編譯器會優先選用移動建構子來取代複製，除非回傳的物件是吃進來的參數
...
看到這裡，大家應該知道為什麼前面的例子中，如果有 `T(T &&other)` 這類的建構子時優先度會比 `T(const T &other)` 還高了，當然你需要提供移動建構子就是了

## 結尾與一些小建議

好啦，這就是全部了，恭喜你讀完了！！

如果各位想要讀近代的 C\+\+，值類別是一個很重要的東西，你會在大量文獻中看到它，那麼這邊 kris 有給了幾個小建議，我在這邊會轉述給大家看，並補一些自己的意見：
...
- 讓 Compiler 幫你做多一點事
    ...
    現在你知道了這些特性，不代表你一定要完美的處理這些複製和移動，像上面講的，如果你在函式中大量的處理了這些管理權的問題，會導致你的可讀性變得很差，而且一沒處理好，你可能還會讓 Compiler 的優化失效，先確保可讀性和可維護性再來講求效
...
- 盡量讓錯誤能在編譯期檢查出來，多使用編譯期運算
    ...
    有興趣的可以看看 [TMP](https://zh.wikipedia.org/wiki/%E6%A8%A1%E6%9D%BF%E8%B6%85%E7%B7%A8%E7%A8%8B)，也是個酷東西，TSJW 有寫過一篇[編譯期算完八皇后所有解](https://tjsw.medium.com/%E6%BD%AE-c-%E7%B7%A8%E8%AD%AF%E6%9C%9F%E7%AE%97%E5%85%AB%E7%9A%87%E5%90%8E%E6%89%80%E6%9C%89%E8%A7%A3-compile-time-8-queens-d872bc063bdc)的 Medium。 但 TMP 也有它的缺點，平衡就看大家的需求自己取捨了

- 讓你的函式短一點

    這會讓你和你的夥伴比較好維護你們寫的 code，然後可讀性也會增加，這是一大好處。 另一大好處是較短的函式能減輕編譯器的負擔，讓編譯器更好的利用前面提到的那些優化來優化你的 code
...
- [62. No unique address attribute](https://timsong-cpp.github.io/cppwp/n4950/dcl.attr.nouniqueaddr)
...
- [86. C++ move semantics on returned value](https://stackoverflow.com/questions/78614644/c-move-semantics-on-returned-value)
